...
/usr/bin/curl --get "$FB_URL" --data 'query=select%20%20like_count%20from%20link_stat%20where%20url=%22http://www.facebook.com/*******%22' > "$OFILE" 2> /tmp/$$-US.log && /bin/rm -f /tmp/$$-US.log || ERROR_FLAG=1
